...
void gyro_signals(void){
  ...
  AccX = (float)AccXLSB/8192 -0.02; //convert ve g
  AccY = (float)AccYLSB/8192 +0.01;
  AccZ = (float)AccZLSB/8192 -0.14;
  ...
  //%Doc gia tri ve tu thanh ghi Gyro
  Wire.beginTransmission(0x68);
  Wire.write(0x43); //Chi ra thanh ghi dau tien se su dung cua bo gia toc goc
  Wire.endTransmission();
...
//  Serial.println(AccZ);
  ...
  dt = 0.05678;
   //%Tinh goc Yaw bang phuong phap tich phan ket hop bo loc Bo sung
  // Chuyển đổi giá trị thô từ accelerometer sang góc
  AccAngle = atan2(AccY, AccZ) * 180 / 3.142; // Góc nghiêng (degree)
  GyroAngle += RateYaw * dt;
  AngleYaw =  (GyroAngle) ;
  //AngleYaw = alpha * (GyroAngle) + (1.0 - alpha) * (AccAngle);
  //Doc gia tri goc
