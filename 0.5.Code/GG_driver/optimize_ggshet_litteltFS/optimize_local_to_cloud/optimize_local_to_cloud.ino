...
const long interval = 50;  // Cập nhật mỗi 50ms (20Hz) //tan so lay mau
...
// Hàm gửi dữ liệu từ file LittleFS lên Google Sheets
void sendDataFromFile() {
  ...
  while (file.available()) {
    ...
    if (separatorIndex != -1) {
      ...
      float temp = line.substring(0, separatorIndex).toFloat();
      float hum = line.substring(separatorIndex + 1).toFloat();

      // Gửi dữ liệu lên Google Sheets
      sendData(temp, hum);
      count++;
      delay(100);  // Để tránh quá tải API của Google Sheets, ta có thể thêm một khoảng thời gian trễ giữa các lần gửi
    }
  }
  file.close();
